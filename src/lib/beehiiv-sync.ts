...
// Sprint 15 Wave 2: extract Ideas right after writing a fresh issue, so
// cron + manual sync keep extracted_ideas aligned with the newsletter
// archive. Best-effort — extraction failures don't unwind the upsert.
import { extractIdeasFromNewsletter } from '@/lib/extract-ideas';
...
export async function upsertIssue(
  ...
  // Wave 2: refresh extracted ideas for this issue after a successful
  // write. Skipped on the no-op short-circuit above so a cron re-sweep
  // of an unchanged archive doesn't re-burn Anthropic credits.
  const titleText = post.title?.trim();
  if (titleText && text && text.trim().length >= 200) {
    try {
      await extractIdeasFromNewsletter({
        userId,
        issueId,
        title: titleText,
        bodyText: text,
      ...
      console.warn('[upsertIssue] extractIdeas failed', post.id, err);
    }
  }
